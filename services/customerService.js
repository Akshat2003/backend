  ...
  async getActiveMembers(filters = {}) {
    try {
      ...
      const query = {
        ...
        'membership.expiresAt': { $gt: new Date() },
        'membership.membershipNumber': { $exists: true }
      };
      ...
      if (search) {
        ...
        query.$or = [
          ...
          { 'membership.membershipNumber': { $regex: searchRegex } }
        ];
      }
      ...
      const sortObj = {};
      if (sortBy === 'expiresAt') {
        sortObj['membership.expiresAt'] = sortOrder === 'asc' ? 1 : -1;
      ...
      const transformedMembers = members.map(member => ({
        ...
        email: member.email,
        vehicles: member.vehicles ? member.vehicles.filter(v => v.isActive) : [],
        membership: {
          ...
          isActive: member.membership.isActive
        },
        bookingStats: member.bookingStats || {
          totalBookings: 0,
          totalRevenue: 0,
          averageDuration: 0
        },
      ...
      logger.info('Active members retrieved successfully', {
        total,
        page,
        limit,
        search,
